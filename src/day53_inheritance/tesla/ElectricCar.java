...
public class ElectricCar {
    ...
    public ElectricCar(String make, String model, double price, int year, int range) {
        setMake(make); // this.make = make; they are the same. setMake is useful when setter has some condition
        this.model = model;
        this.price = price;
        this.year = year;
        this.range = range;
        count++; //increase count by 1 - everytime when
    }
    ...
    public void setMake(String make) {
        if (make.isEmpty()){
            System.out.println("ERROR: Make cannot be a blank");
        }else{
            this.make = make;
        }
        this.make = make;
